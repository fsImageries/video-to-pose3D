...
class Timer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.show:
            print(
                f'{self.message} --- elapsed time: {time.perf_counter() - self.start} s')
...
def add_path():
    ...
    paths = filter(lambda p: p not in sys.path, [
                   Alphapose_path, hrnet_path, trackers_path])
...
def change_score(prediction, detectron_detection_path):
    detectron_predictions = np.load(detectron_detection_path, allow_pickle=True)[
        'positions_2d'].item()
...
def read_video(filename, fps=None, skip=0, limit=-1, logger=None):
...
def split_video(video_path):
    ...
    save_folder = pathlib.Path(
        f'./{output_dir}/alpha_pose_{video_name}/split_image/')
