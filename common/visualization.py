...
def ckpt_time(ckpt=None, display=0, desc=''):
    if not ckpt:
        ...
        if display:
            if logger:
                logger.info(desc + ' consume time {:0.4f}'.format(time.time() - float(ckpt)))
            else:
...
def render_animation(keypoints, poses, skeleton, fps, bitrate, azim, output, viewport,
    ...
    if input_video_path is None:
        ...
        all_frames = []
        for f in read_video(input_video_path, fps=None, skip=input_video_skip, logger=logger):
            all_frames.append(f)
    ...
    def update_video(i):
        ...
        if not initialized:
            ...
            points = ax_in.scatter(
                *keypoints[i].T, 5, color='red', edgecolors='white', zorder=10)
            ...
            for j, j_parent in enumerate(parents):
                ...
                for n, ax in enumerate(ax_3d):
                    pos = poses[n][i]
                    # Hotfix matplotlib's bug. https://github.com/matplotlib/matplotlib/pull/20555
                    lines_3d[n][j -
    ...
    anim = FuncAnimation(fig, update_video, frames=limit,
