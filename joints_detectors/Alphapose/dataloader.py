        ...
        for i in range(self.num_batches):
            ...
            for k in range(i * self.batchSize, min((i + 1) * self.batchSize, self.datalen)):
                ...
                if not grabbed:
                    ...
                    opt.logger.info('===========================> This video get ' +
        ...
        for i in range(self.num_batches):
            ...
            with torch.no_grad():
                ...
                prediction = self.det_model(
                    img, CUDA=torch.cuda.is_available())
                # NMS process
    ...
    def update(self):
        ...
        for i in range(self.num_batches):
            ...
            with torch.no_grad():
                ...
                wd = inp[0].size(2)
                # Human Detection
                img = Variable(torch.cat(img)).to(get_device())
                # img = Variable(torch.cat(img)).cuda()
                im_dim_list = torch.FloatTensor(im_dim_list).repeat(1, 2)
                im_dim_list = im_dim_list.to(get_device())
...
def crop_from_dets(img, boxes, inps, pt1, pt2):
    ...
    for i, box in enumerate(boxes):
        ...
        try:
            ...
            # print(bottomRight)
            # print('===')
            opt.logger.info(tmp_img.shape)
            opt.logger.info(upLeft)
            opt.logger.info(bottomRight)
            opt.logger.info("==")
        pt1[i] = upLeft
        pt2[i] = bottomRight
